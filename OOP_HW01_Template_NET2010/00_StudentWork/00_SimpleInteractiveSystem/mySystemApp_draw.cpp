...
void MY_SYSTEM_APP::draw_QuadraticFunction( ) const
{
	glLineWidth(2.0);
	glColor3f(0.3, 0.3, 0.8 );

	double x0, x1;
	mQuadraticFunction.getRangeOfX( x0, x1 );
	glBegin(GL_LINE_STRIP );
	int numSamples = mQuadraticFunction.getNumOfSamples( );
	for ( int i = 0; i < numSamples; ++i ) {
		float y;
		float x;
		x = x0 + (i/((double) numSamples -1))*(x1 - x0);
		y = mQuadraticFunction.getValue( x );
		glVertex3f( x, 0.0, y );
	}
	glEnd( );
}
