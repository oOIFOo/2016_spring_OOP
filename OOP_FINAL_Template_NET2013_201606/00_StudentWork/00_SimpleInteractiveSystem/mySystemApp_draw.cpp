...
*/
	...
	for (int j = 0; j < 2; ++j) {
		if ( j == 0) glColor3f(0.0, 0.0, 0.0);
		else glColor3f(0.5, 0.5, 0.5);
		glBegin(GL_LINE_STRIP);
		for (int i = 0; i < numSegments + 1; ++i) {
			double r0 = i*f;
			double x0 = minX + r0*(maxX - minX);
			double y0 = mMonteCarloSystem.getValue(x0, j);
    ...
    double ani_y = mMonteCarloSystem.getValue( ani_x, 0 );
...
void MY_SYSTEM_APP::draw_GalaxySystem( ) const
{
	...
	for ( int i = 0; i < numObjs; ++i ) {
		...
		double x;
        double r;
        mGalaxySystem.getObjInfo(i, x, y, r);
		double f = i / (double)numObjs;
